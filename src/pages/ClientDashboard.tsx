...
import { GET_ME, GET_CLIENT_STATS, GET_RIDE_HISTORY } from '../api/queries';
  ...
  <div className="col-span-full bg-gradient-to-br from-primary-dark via-slate-900 to-black rounded-3xl border border-white/10 overflow-hidden relative shadow-2xl flex flex-col md:flex-row">
    <div className="p-8 md:w-1/2 flex flex-col justify-center relative z-10">
      <div className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/50 text-green-400 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-6 w-max animate-pulse">
        <div className="w-2 h-2 rounded-full bg-green-400" /> Ride in Progress
      </div>
      <h3 className="text-3xl font-black text-white mb-2">Heading to {ride.destination}</h3>
      {ride.estimatedArrival && (
        <p className="text-slate-400 text-sm mb-8 flex items-center gap-2">
          <Clock size={16} /> Est. Arrival: {ride.estimatedArrival}
        </p>
      )}
      <div className="space-y-4">
        <div className="flex gap-4">
          <div className="w-5 h-5 rounded-full bg-slate-700 border-2 border-slate-500 flex-shrink-0 mt-1" />
          <div><p className="text-xs text-slate-500 font-bold uppercase">Pickup</p><p className="text-white font-medium">{ride.pickup}</p></div>
        </div>
        <div className="flex gap-4">
          <div className="w-5 h-5 rounded-full bg-primary-light border-2 border-primary-light/40 flex-shrink-0 mt-1" />
          <div><p className="text-xs text-primary-light font-bold uppercase">Destination</p><p className="text-white font-medium">{ride.destination}</p></div>
        </div>
      </div>
    </div>
      ...
      <div className="p-8 md:w-1/2 bg-white/5 backdrop-blur-md border-l border-white/10 flex flex-col justify-between relative z-10">
        <div className="bg-black/50 rounded-2xl p-4 border border-white/5 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h4 className="text-white font-bold text-lg">{ride.rider.fullName}</h4>
            <p className="text-slate-400 text-sm flex items-center gap-1">
              <Star size={14} className="text-amber-500 fill-current" /> {Number(ride.rider.rating).toFixed(1)} ({ride.rider.totalTrips} trips)
            </p>
            {ride.rider.plateNumber && <p className="text-xs text-slate-500 mt-1">{ride.rider.plateNumber}</p>}
          </div>
        </div>
        <div className="mt-6 flex gap-4">
          ...
          <button className="w-14 h-14 bg-red-500/20 text-red-500 border border-red-500/50 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
            <Shield size={20} />
          </button>
        </div>
      </div>
...
const RideHistoryTable: React.FC<{ clientId?: string }> = () => {
  ...
  const { data, loading, error } = useQuery(GET_RIDE_HISTORY, {
    ...
    <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl overflow-hidden flex flex-col h-[500px] md:h-[600px]">
              ...
              {filtered.map((ride: any) => (
                <tr key={ride.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 hidden lg:table-cell text-sm text-slate-600 dark:text-slate-400">
                    ...
                    <div className="text-xs mt-1 text-slate-400">{ride.formattedDuration}</div>
                  </td>
                  <td className="px-6 py-4">
                    <p className="text-sm font-black text-slate-900 dark:text-white">TZS {ride.formattedAmount}</p>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-white/10 flex justify-between items-center bg-slate-50 dark:bg-transparent">
        <p className="text-xs font-bold text-slate-500">
          Showing {Math.min((page - 1) * PAGE_SIZE + 1, total)}–{Math.min(page * PAGE_SIZE, total)} of {total} rides
        </p>
        <div className="flex gap-1">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
            className="w-8 h-8 rounded border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-400 disabled:opacity-40 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors">
            <ChevronLeft size={14} />
          </button>
          {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
            <button key={p} onClick={() => setPage(p)}
              className={`w-8 h-8 rounded flex items-center justify-center text-sm font-bold transition-colors ${page === p ? 'bg-primary-light text-white' : 'border border-slate-200 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/5'}`}>
              {p}
            </button>
          ))}
  ...
  <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white border border-white/10 rounded-3xl p-8 relative overflow-hidden shadow-2xl flex flex-col h-full">
    ...
    <div className="flex justify-between items-start mb-8 relative z-10">
      <div>
        ...
        {loading ? (
          ...
          <h3 className="text-4xl font-black tracking-tight">TZS {(totalSpent / 1000).toFixed(0)}k</h3>
        )}
      </div>
    ...
    <div className="mt-auto relative z-10">
      ...
      <div className="space-y-3">
        {['M-Pesa', 'Airtel Money', 'BodaWallet'].map((pm, i) => (
          <div key={pm} className="flex items-center justify-between p-3 rounded-xl bg-black/40 border border-white/5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">
                <Smartphone size={18} className="text-green-400" />
              </div>
              <p className="font-bold text-sm">{pm}</p>
            </div>
            {i === 0 && <span className="bg-primary-light/20 text-primary-light text-[10px] font-black uppercase px-2 py-1 rounded">Default</span>}
          </div>
        ))}
        <button className="w-full py-3 rounded-xl border border-dashed border-white/20 text-sm font-bold text-slate-300 hover:bg-white/5 transition-colors">
          + Add Payment Method
        </button>
      </div>
    </div>
...
const ClientDashboard: React.FC = () => {
  ...
  const { data: statsData, loading: statsLoading } = useQuery(GET_CLIENT_STATS, {
    pollInterval: 30000, // refresh every 30s to catch active ride updates
  });
    ...
    <DashboardLayout role="client">
      ...
      <div className="max-w-7xl mx-auto space-y-8 w-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            {meLoading ? (
              <div className="h-10 w-64 bg-slate-200 dark:bg-white/10 rounded-2xl animate-pulse mb-3" />
            ) : (
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">
                {getGreeting()},{' '}
                <span className="text-primary-light">{user?.fullName?.split(' ')[0] ?? 'there'}</span>
              </h1>
            )}
            <p className="text-slate-600 dark:text-slate-400 mt-2 font-medium">
              {stats
                ? `${stats.totalRides} rides completed. Thank you for choosing BodaKitaa!`
                : 'Welcome back to BodaKitaa!'}
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard loading={statsLoading} title="Total Rides" value={stats?.totalRides ?? '—'} icon={MapPin} color="text-blue-500" />
          <StatCard loading={statsLoading} title="Total Spent" value={stats ? `TZS ${(stats.totalSpent / 1000).toFixed(0)}k` : '—'} icon={CreditCard} color="text-green-500" />
          <StatCard loading={statsLoading} title="Loyalty Points" value={stats?.loyaltyPoints ?? '—'} icon={Star} color="text-amber-500" subtitle="Earn 100 pts/ride" />
          <StatCard loading={statsLoading} title="CO₂ Saved" value={stats ? `${stats.carbonSaved} kg` : '—'} icon={Activity} color="text-primary-light" subtitle="vs. car travel" />
        </div>
